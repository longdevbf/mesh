import {
  ...
  async function main() {
    try {
      ...
      const storeAddr = serializePlutusScript(
            { code: storeScriptCbor, version: "V3" },
            undefined,
            0,
          ).address;
      console.log("storeScript : ", storeAddr);
    ...
    const storeScriptHash = deserializeAddress(storeAddr).scriptHash;
