...
exports.blockchainProvider = new core_1.BlockfrostProvider('previewMh4GFYODhhPNCM4CsbptHtiHpcvBJRRP');
...
function readValidator(title) {
    var plutusPath = path.join(__dirname, 'plutus.json');
    var plutusJson = JSON.parse(fs.readFileSync(plutusPath, 'utf-8'));
    var validator = plutusJson.validators.find(function (v) { return v.title === title; });
    if (!validator) {
